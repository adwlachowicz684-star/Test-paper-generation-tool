...
def run(chars, lines):
    ...
    for L in sorted(lines, key=lambda t: (t['y'], t['x0'])):
        ...
        fracs.append({
            ...
            'y0': min(c['y0'] for c in num),   # 分子顶部（仅作兜底）
            'ly': L['y'],                      # ★ 分数线 y，归属用它
            'txt': r'\frac{%s}{%s}' % (
    ...
    # ★ 归属到「分数线 y 落在哪一行」—— 用分子顶部会归错行：
    # 分子在上一行的底部，按 y0 匹配会把 \frac 插到上一行末尾，
    # 实测出现「∴a ≤ 或a ≥ ．\frac{x}{e^{x}}」这种断裂。
    # 分数线本体就在公式主体行内，用它的 y 判断才对。
    bucket = [[] for _ in rows]
    for f in fracs:
        best, bd = None, 1e9
        for i, row in enumerate(rows):
            ry0 = min(c['y0'] for c in row)
            ry1 = max(c['y1'] for c in row)
            if ry0 - 4 <= f['ly'] <= ry1 + 4:      # 落在该行内
                d = abs(f['ly'] - (ry0 + ry1) / 2)
                if d < bd:
                    bd, best = d, i
        if best is None:                            # 兜底：取 y 最接近的行
            for i, row in enumerate(rows):
                d = abs(f['ly'] - min(c['y0'] for c in row))
                if d < bd:
                    bd, best = d, i
