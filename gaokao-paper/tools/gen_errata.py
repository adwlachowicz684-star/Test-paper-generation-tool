...
A_MANUAL = [
    ...
    # 第137批：M-T-185-V1（p149 第 33 题）
    ('M-H1412', 'M-T-185-V1', '$8$', '**$24$**',
     r'代入 $a=4,\,b=0,\,c=-4$ 即得 $\\lvert4-0\\rvert+\\lvert0+4\\rvert+2\\lvert-4-4\\rvert=4+4+16=24>8$，'
     r'原书答案连可行值都不是最大值。正确上界分三种情况：'
     r'$c\\le b\\le a$ 时原式 $=3(a-c)\\le24$（$b$ 完全消失）；'
     r'$b>a\\ge c$ 时 $=2b+a-3c\\le24$；$b<c\\le a$ 时 $=3a-2b-c\\le24$。'
     r'**原书详解三处连环错**：'
     r'(i) 写「$x=a-b,\,y=b-c,\,z=a-c$，故 $x^2+y^2=z^2$」——实际是 $x+y=z$（三角不等式取等），'
     r'而非勾股关系；(ii) 写「$0\\le z\\le2\\sqrt2$」——由 $a,c\\in[-4,4]$ 只能得 $z\\le8$，$2\\sqrt2$ 无来源；'
     r'(iii) 末步「$z(\\sqrt2+2)=2\\sqrt2\\times2\\sqrt2=8$」——既用了错的 $z$ 上界，'
     r'又把 $\\sqrt2+2\\approx3.414$ 误当成 $2\\sqrt2\\approx2.828$；'
     r'按它自己的式子也该得 $2\\sqrt2(\\sqrt2+2)=4+4\\sqrt2\\approx9.66\\ne8$。'
     r'随机 $2\\times10^6$ 组采样最大 $=23.9901$，端点 $(4,0,-4)$、$(4,4,-4)$、$(4,-4,-4)$ 均精确取到 $24$'),
]
