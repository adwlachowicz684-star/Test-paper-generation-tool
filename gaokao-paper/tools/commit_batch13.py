# -*- coding: utf-8 -*-
r"""第12批入库：T023 超难压轴小题 / T066 一元二次复合型基础，共 8 题。
...
def main():
    ...
    for d in QS:
        ...
        qs.append({
            'subject': '数学',
            'subtype': d.get('subtype') or ('双空题' if d['answer'].count('；') else ''),
            'type': d['type'],
            'stem_text': d['stem_text'],
            'opts': [list(o) for o in d['opts']],
            'answer': d['answer'],
            # solution_ext / solution_ext2 是为了绕开「raw 字符串里写一半又开新串」
            # 的引号坑，这里合并成完整解析。
            'solution': d['solution']
                       + (d.get('solution_ext') or '')
                       + (d.get('solution_ext2') or ''),
